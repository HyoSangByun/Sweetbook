...
## 1. 서비스 소개
- 한 줄 소개: 러닝 활동 데이터를 기반으로 SweetBook 포토북을 생성하고 주문까지 연결하는 서비스입니다.
...
| AI 도구 | 활용 내용 |
|---|---|
| Claude Code | 백엔드 API 라우팅 구조 설계 및 DTO/서비스 리팩토링 아이디어 검토 |
| ChatGPT | 프론트 단계별 UX 개선안, 에러 원인 분석, 문서화(README) 초안 작성 |

## 5. 설계 의도
- 왜 이 서비스를 선택했는지:
  - 러닝 데이터는 누적 가치가 높고, 결과물을 실물 포토북으로 전환하면 사용자 만족도와 재구매 가능성이 큽니다.
- 비즈니스 가능성:
