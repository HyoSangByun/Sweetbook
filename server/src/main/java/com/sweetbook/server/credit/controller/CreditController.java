...
@Tag(name = "Credits", description = "크레딧 잔액 조회/충전 API")
public class CreditController {
    ...
    @Operation(summary = "Sandbox 크레딧 충전", description = "테스트 목적으로 sandbox 크레딧을 충전합니다.")
    public ResponseEntity<ApiResponse<ChargeCreditResponse>> chargeSandboxCredits(
            @Valid @RequestBody ChargeCreditRequest request
    ) {
        return ResponseEntity.ok(ApiResponse.ok(creditService.chargeSandboxCredits(request)));
    }
}
